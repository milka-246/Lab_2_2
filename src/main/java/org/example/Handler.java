...
{
    HashMap <String, Integer> registr = new HashMap<>();
    int size = 1024;
    ...
    @Override
    public void work (Command com) throws Exception_div_zero, Exception_exceeded_index, Exception_missing_register, Exception_unidentified_command
    {
        try
        {
    ...
    private void ld (String in, int index) throws Exception_exceeded_index, Exception_missing_register
    {
        ...
        /*if(in.equals("a")) {a = memory[index];}
        if(in.equals("b")) {b = memory[index];}
        if(in.equals("c")) {c = memory[index];}
        if(in.equals("d")) {d = memory[index];}
        */
    }
